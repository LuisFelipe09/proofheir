...
function StepIndicator({ steps, currentStep }: { steps: { id: string; title: string }[]; currentStep: number }) {
    ...
    return (
        <nav aria-label="Registration Progress" className="mb-6 sm:mb-8">
            <div className="relative px-2 sm:px-0">
                {/* Progress line background */}
                <div aria-hidden="true" className="absolute top-4 sm:top-5 left-6 right-6 sm:left-5 sm:right-5 h-0.5 bg-slate-700" />
                {/* Progress line fill */}
                <div
                    aria-hidden="true"
                            ...
                            >
                                ...
                                >
                                    {isCompleted ? (
                                        ...
                                        <>
                                            <span className="sr-only">{isCurrent ? 'Current step: ' : 'Upcoming step: '}</span>
                                            {index + 1}
                                        </>
                                    )}
                                </div>
