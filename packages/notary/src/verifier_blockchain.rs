...
pub async fn verifier<T: AsyncWrite + AsyncRead + Send + Sync + Unpin + 'static>(
    ...
    // DEBUG: Log the public inputs that will be serialized
    tracing::info!("📊 DEBUG - Proof id_commitment: {:?}", msg.public_inputs.id_commitment);
    ...
    tracing::info!("📊 DEBUG - Receipt status(): {}", receipt.status());
    tracing::info!("📊 DEBUG - Receipt block: {:?}", receipt.block_number);
    tracing::info!("📊 DEBUG - Receipt gas_used: {}", receipt.gas_used);
